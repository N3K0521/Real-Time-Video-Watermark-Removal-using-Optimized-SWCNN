...
def main():
    ...
    #
    dataset_train = Dataset(train=True, mode='color', data_path=config['train_data_path'], patch_size=256, stride=128, aug_times=1)
    ...
    # print(dataset_train.shape, dataset_val.shape)
    print(len(dataset_train),len(dataset_val))
    
    loader_train = DataLoader(dataset=dataset_train, num_workers=8, batch_size=opt.batchSize, shuffle=True,pin_memory=True)  # 4
    ...
    for epoch in range(opt.epochs):
        ...
        for i, data in enumerate(loader_train, 0):
            ...
            optimizer.zero_grad()
            img_train = data
            # print(torch.max(img_train),torch.min(img_train))
        ...
        with torch.no_grad():
            for k in range(len(dataset_val)):
                ...
                if k == 0:  # Only log the first batch of validation images to save space
                    # Move tensors to CPU for visualization
                    # img_val_cpu = img_val.cpu()
                    # imgn_val_cpu = imgn_val.cpu()
                    # out_val_cpu = out_val.cpu()

                    # Add images to TensorBoard
                    # Concatenate images along the width (dim=3)
                    combined_image = torch.cat((img_val[0], imgn_val[0], out_val[0]), dim=2).cpu()

                    # Add concatenated image to TensorBoard
                    writer.add_image('Val/Output_Image', combined_image, epoch + 1)
